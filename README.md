...
| `--max-open-risk R` | **Portfolio open-risk budget** (default `6.0`), in `size_mult` units. Each open position consumes its own `size_mult`, so a new entry is dropped once total open risk would exceed the budget — a half-size (regime/chronic-reduced) position uses half a slot. A risk control, so it is universe-agnostic (not a raw count). Lower → fewer concurrent positions. | `portfolio.max_open_risk` (`base_port`) |
