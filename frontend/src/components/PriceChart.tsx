...
// Candlestick + Bollinger + MA fast/slow, with a synced RSI and MACD pane below.
// Ported from the proven inline-SVG renderer; no external chart dependency.
...
// The SVG strings below interpolate ONLY numeric coordinates/values (via toFixed)
// and server-generated date labels — never free text — so they are safe to inject.
function buildModel(bars: Bar[]): Model | null {
  const n = bars.length;
  if (n < 2) return null; // need >=2 bars; n<2 would divide by (n-1)=0 -> NaN coords
  ...
  const Tp = 10,
    B = 184;
  const all = cs.flat().concat(bu.filter(fin), bl.filter(fin));
  ...
  const ln = (a: (number | null)[], col: string, w: number) =>
    `<polyline points="${fpts(a)}" style="fill:none;stroke:${col};stroke-width:${w}"/>`;
  ...
  const candlesSvg = `<svg viewBox="0 0 ${VB} 210" width="100%">${grid}${xl}${bbA ? `<path d="${bbA}" style="fill:var(--bg-accent);opacity:.18"/>` : ""}${cnd}${ln(bu, "var(--text-muted)", 0.6)}${ln(bl, "var(--text-muted)", 0.6)}${ln(maf, "var(--text-accent)", 1.3)}${ln(mas, "var(--text-warning)", 1.3)}${yl}</svg>`;
  ...
  // RSI line uses its own 0-100 pane scale (RY), not the price scale.
  ...
  // MACD + signal lines use the MACD pane scale (MY), not the price scale.
  ...
  return { n, dts, cs, candlesSvg, rsiSvg, macdSvg };
}
